...
class Article_service extends MY_Service {
	...
	public function read_addoredit_ajax($options = array()){
		...
		$data = array();
		if (optionExists($pid)) {
			$list = $this->ArticleAdd_model->where('pid',$pid)->get_item();
			$data['readnum'] = $list['readnum']+1;
			$success = $this->ArticleAdd_model->set($data)->where("pid", $pid)->update();
			if (!$success) {
	...
	public function get_prev($article) {
		return $this->format($this->Article_model->select("article.*")->where(array(
	...
	public function get_list($options) {
		...
		if (optionExists($limit) && optionExists($page)) {
			$total = $this->Article_model->where($where)->count();
			if ($wap) {
				$pages = pages_m($total, $limit, formatUrl("channel_m", "channel", ''));
			} else {
				$pages = pages($total, $limit, formatUrl("channel", "channel", ''));
			}
			$offset = ($page - 1) * $limit;
		}
		...
		$list = $this->Article_model->where($where)->order_by("article.id", "DESC")->get_list();
		foreach($list as $k=>$v){
			$add = $this->ArticleAdd_model->where('pid',$v['id'])->get_list();
			foreach($add as $v){
				$list[$k]['editor'] = $v['editor'];
			}
		}
	...
	public function insertorupdate($data) {
		...
		return $id ? success_msg($success) : success_msg(array(
			"cname" => $dir,
			"aid" => $pid,
			"pc" => formatUrl('article', $data['utype'] == 1 ? $dir : $data['uid'], $pid),
			"wap" => formatUrl('article_m', $data['utype'] == 1 ? $dir : $data['uid'], $pid)
		));
	}
	...
	public function status($id, $status) {
		...
		$success = $this->Article_model->set(array("status" => $status))->where("id", $id)->update();
		if (!$success) {
			return wrrong_msg(MSG_SERVICE_BUSY);
		}
		return success_msg(MSG_METHOD_SUCCESS);
